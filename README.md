...
build_linux_arm:
        @GOOS=linux go build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
                -o bin/linux/arm64/helmpush cmd/helmpush/main.go  # linux arm

build_link_linux:
        @cp bin/linux/arm64/helmpush ./bin/helmpush
...
```

vim scripts/install_plugin_arm.sh
```
#!/bin/sh -e
...
#  update: "cd $HELM_PLUGIN_DIR; scripts/install_plugin.sh"
  install: "cd $HELM_PLUGIN_DIR; scripts/install_plugin_arm.sh"
  update: "cd $HELM_PLUGIN_DIR; scripts/install_plugin_arm.sh"
```
